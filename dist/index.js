 ...
 *
 * Creates an access request in SailPoint IdentityNow to grant access to roles,
...
var script = {
  ...
  invoke: async (params, context) => {
    ...
    if (response.ok) {
      ...
      console.log(`Successfully created access request ${responseData.id} for identity ${identityId}`);
    ...
    let errorMessage = `Failed to create access request: HTTP ${statusCode}`;
    ...
    try {
      ...
      if (errorText) {
        errorMessage = `Failed to create access request: ${errorText}`;
      }
  ...
  halt: async (params, _context) => {
    ...
    console.log(`Access request job is being halted (${reason}) for identity ${identityId}`);
